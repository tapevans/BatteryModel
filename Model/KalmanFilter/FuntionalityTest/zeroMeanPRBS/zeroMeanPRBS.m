...
%% Make PRBS Signal
    ...
    C_Demand = PRBS_gen(PP : PRBSLength)*PRBSAmp;  % PRBS Demand C [A/m^2]
    t_Demand = ((0 : 1 : PRBSLength-PP)*(Tswitch))'; % PRBS Demand t [s]
...
%% New PRBS Signal
    ...
    figure
    hold on
    plot(t_vec(1:idx),i_user(1:idx),'k','Linewidth',2)
    yline(0,'r','Linewidth',2)
    title(['New PRBS Signal'])
    xlim([0,t_vec(idx)])

    figure
    hold on
    plot(t_vec(1:idx),coulumbs(1:idx),'k','Linewidth',2)
    yline(0,'r','Linewidth',2)
    title(['New PRBS Current'])
    xlim([0,t_vec(idx)])
