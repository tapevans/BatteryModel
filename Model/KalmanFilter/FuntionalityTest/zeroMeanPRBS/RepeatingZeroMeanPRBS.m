...
%% Make PRBS Signal
    ...
    if MakeLongPRBSSignal
        num_repeats = ceil(TotalSamples/length(C_Demand));
        % num_repeats = 4;
    ...
    t_Demand = ((1:length(C_Demand))*(Tswitch))' - 1;
...
%% Plot Results
    ...
    xlim([t_vec(end)-100,t_vec(end)])

    disp(['Integration of charge results in '])
    disp(num2str(coulombs(end)));
