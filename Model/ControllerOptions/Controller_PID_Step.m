...
% 2) Constant Voltage
% 3) Relaxation
function [i_user] = Controller_PID_Step(t, SV, AN , SEP, CA , EL , P , N , SIM , CONS , FLAG , PROPS)
% Parameters
	K_p = 0.5;
	K_i = 0.5;
	K_d = 0.5;
...
%         i_user = i_user_history(end) - i_user;
	elseif MO == 3 % Relaxation
		i_user = 0;
	else
		disp('do not know what mode of operation I am in AGAIN')
...
%% i_user Saturation
    i_user_min_max = SIM.Controller_MO_File(current_MO_step).C_rate_sat*SIM.Cell_Cap/SIM.A_c;
    if i_user > i_user_min_max
        i_user = i_user_min_max;
    elseif i_user < -i_user_min_max
        i_user = -i_user_min_max;
    end
