...
if FLAG.READ_IN_DATA
    % Get filenames
    oldFolder = cd([pwd filesep 'Step_CodedPlantResultsQInput']);
    list = dir('*.mat*');
...
hold on
for j = idx'
    scatter(MyData(j).SVD , log10(MyData(j).CPC),'filled',color_vec{MyData(j).C},'MarkerEdgeColor','k')
end
title(['Q = ' num2str(Q_0) '  R = ' num2str(R_0) '  Covariance: ' num2str(Covar) ])
xlabel('Normalized Singular Values (Of SV)')
ylabel('log10(Error Covariance)')
exportgraphics(gca,'ColoredCorr.png','Resolution',800)
