...
%
%%
function [x_hat,K_k,P_k_pre,P_k] = VarEstimator(sys,FLAG,SIM,P,x_hat_0,u,z)
    u_k = reshape(u,1,[]);
    z_k = reshape(z,1,[]);
...
%% Initialize Variables
    ...
    P_k_pre(:,:,1) = P_0;
    S_k(:,:,1)     = C_DT_CV * P_k_pre(:,:,1) * C_DT_CV' + R;
    K_k(:,:,1)     = P_k_pre(:,:,1) * C_DT_CV' * inv(S_k(:,:,1));
    P_k(:,:,1)     = (eye(N_states) - K_k(:,:,1) * C_DT_CV) * P_k_pre(:,:,1);
    % P_k(:,:,1)     = P_0;
...
%% OOOOOOOOOOOOOOOOOOOOOOOOOOOLLLLLLLLLLLLDDDDDDDDDDDDDDDDD
